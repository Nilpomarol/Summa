-- Summa v14 -> v15 migration.
-- Shared-account membership, patrimonial ownership, expense financing and contributions.
-- Percentages use integer basis points so every cent calculation is deterministic.
...
CREATE UNIQUE INDEX idx_account_members_one_user ON account_members(account_id)
    WHERE participant_kind = 'user' AND archived_at IS NULL;
CREATE UNIQUE INDEX idx_account_members_one_person ON account_members(account_id, person_id)
    WHERE participant_kind = 'person' AND archived_at IS NULL;

ALTER TABLE movements ADD COLUMN expense_funding TEXT
    CHECK (expense_funding IN ('owner','shared_account'));
UPDATE movements SET expense_funding = 'owner'
WHERE type = 'expense' AND expense_funding IS NULL;
