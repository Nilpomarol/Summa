-- Summa v16 -> v17 migration.
-- Money leaves a shared account the same way it enters, and income stops being wholly the app
-- owner's just because it landed in a shared account.

-- Member money now moves both ways. Existing rows are contributions, so they default to 'in'.
...
-- An income allocation is a split like any other, so debt has to name the movement type it means.
