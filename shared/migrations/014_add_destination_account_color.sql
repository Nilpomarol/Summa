-- Summa v13 -> v14 migration.
-- Exposes the destination account's colour on v_movement_summary, completing what v13 started:
-- a transfer names two accounts, so showing only the source one in colour left the pair
-- half-identified. destination_accounts was already joined by the view -- only its name was
-- selected -- so this adds no join, no row and no money semantics. The external-expense branch
-- has no accounts at all, so the column is NULL there exactly as destination_account_name
-- already is. Semicolons are deliberately kept out of comments so naive statement splitters
-- cannot mis-cut.
