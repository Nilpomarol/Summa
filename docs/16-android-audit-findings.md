...
| **C9** | Movement form type/payer switches silently destroy entered data | Critical | P5R-17 | `docs/17` WP1 — segment re-tap guard, non-destructive normalization, pre-save data-loss warning | High |
| **F12** | Movement form misrepresents recurrence on edit | Functional | P5R-17 | `docs/17` WP3 — load template frequency, read-only when linked, toggle-off offers end-template, quick-template carries notes | High |
| **F13** | Budgets unreachable through normal navigation | Functional | P5R-17 | `docs/17` WP4 — Gestió tile + category-flow CTA; notification retarget | High |
...
| **U8** | Form validation errors render off-screen, no field identity | Usability | P5R-17 | `docs/17` WP2 — field-level `isError` + scroll-to-error; duplicate-warning placement | High |
| **U9** | Four forms still use free-text `YYYY-MM-DD` date entry | Usability | P5R-17 | `docs/17` WP5 — `FormDatePicker` in refund/settlement/confirm-due/budget forms | High |
| **U10** | Budget archive fires with no confirmation | Usability | P5R-17 | `docs/17` WP4 — standard `archiveCandidate` dialog | High |
| **U11** | Settlements/refunds uneditable, with no explanation shown | Usability | P5R-17 | `docs/17` WP6a (hint) now; WP6b (edit support) optional, schema-steward-gated | Medium |
...
Findings from a dedicated UX audit of every add/edit flow (movement form + detail, accounts, categories, recurring, trips, tags, people/settlements, refunds, budgets, navigation shell), performed 2026-07-10 after `P5R-16`. All are owned by **`P5R-17`**; full evidence, impact, and step-level remediation live in `docs/17-data-entry-ux-remediation.md`, whose work packages (WP1–WP11) each name the IDs they own — the master table (§3) carries the one-line summary per ID. Only the critical one gets its own note here:
...
The rest (F12–F14, U8–U16, M16) are functional gaps, feedback/consistency problems, and friction — none data-corrupting on their own, but F12 (recurrence edits silently ignored) and F13 (budgets undiscoverable) are features that silently don't work, which is why this slice runs before the phase-close `P5R-11`/`P5R-12` checks rather than being deferred to P8.
...
- **Tier 3 mapping:**
  ...
  - **P5R-17** (data-entry UX remediation, `docs/17-data-entry-ux-remediation.md`): C9, F12, F13, F14, U8–U16, M16 — WP-level ownership mapped in that doc (WP1=C9, WP2=U8, WP3=F12, WP4=F13+U10, WP5=U9, WP6=U11, WP7=U12+M16, WP8=U13+F14, WP9=U14, WP10=U15, WP11=U16).
