...
| **M7** | Dead `AnalysisRepository` methods + unused composable | Maintainability | **PARTIALLY RESOLVED** P5R-9 | `dailyIncomeVsExpense` (a redundant pure-Kotlin wrapper around `incomeVsExpense(bucket = DAY)`, no dedicated SQL) and `NoDataText()` deleted. `accountFlowOverTime`/`largestExpenses`/`topMerchants` were **not** deleted on closer look: they back onto canonical `shared/queries/analysis_*.sql` files still documented as intended features in `docs/00-Full_Spec.md`, `docs/03-architecture.md`, and roadmap `P2-2`/`P2-8` — they're a P5R-4 Analysis-rebuild UI gap (never wired into the 5-tab redesign), not dead code, and deleting spec'd shared-contract SQL is a schema-steward-level call outside this cleanup slice's scope | Low |
| **M8** | Duplicated percent-formatting logic across Analysis | Maintainability | **RESOLVED** P5R-9 | Extracted `formatPercentLabel(fraction)` to `ui/common/MoneyText.kt`; `DashboardScreen.kt`, `AnalysisWidgets.kt` (incl. deleting the redundant `formatRecurringCostSharePercent` wrapper), `CategoriesTab.kt`, `FixVariableTab.kt` (incl. deleting `formatGroupPercent`), and `TripsScreen.kt` (same logic, found during the sweep) all now call it. `CategoriesScreen.kt`/`AccountsScreen.kt` use a *different* threshold+rounding convention (`0.01f`+truncate vs `0.005f`+round) and were deliberately left alone — unifying them would silently change visible rounding on those two screens, out of scope for a no-behavior-change dedup | Low |
| **M9** | ~21 orphaned strings in Core Ledger area | Maintainability | **RESOLVED** P5R-9 | Removed 23 confirmed-unreferenced entries (`account_detail_*`, `account_action_*`, most of `account_flow_*`, `category_parent_value`, `movement_detail_*`, `movement_action_edit/archive`) after grepping the whole app for each name individually; `account_flow_loading/empty/view_analysis` and the `account_flow_movement_count` plural are still used and were kept | Low |
| **M10** | Duplicate `ColorDot` composables | Maintainability | **RESOLVED** P5R-9 | `MovementsScreen.kt`'s private `ColorDot` (+ its only-user `PillShape`) deleted; its one call site now uses `FormControls.kt`'s `ColorDot(colorHex = account.color, size = 12.dp)` — both call sites pass the same `account.color` field, so this also fixes an incidental inconsistency where the same account-color dot resolved through two different fallback colors depending on which screen rendered it | Low |
| **M11** | Duplicate `displayOrder`+name sort logic | Maintainability | **RESOLVED** P5R-9 | New `List<T>.sortedByDisplayOrderThenName(displayOrder, name)` in `ui/common/ListSorting.kt`; `AccountsViewModel.kt` and `CategoriesScreen.kt` both call it | Low |
| **M12** | Duplicate `MovementType.label()` in `RecurringScreen` | Maintainability | **RESOLVED** P5R-9 | Local duplicate (byte-identical to `ui/common/Labels.kt`'s version) deleted; `RecurringScreen.kt` now imports the shared one | Low |
| **M13** | Duplicated split-mapping logic in `SplitRepository` | Maintainability | **RESOLVED** P5R-9 | `getForMovement`/`getForMovementById` both pass a shared private `mapSplitLineRow(...)` by reference to their SQLDelight query (matching the existing `mapAccountFlowEntry` idiom already reused across `accountFlowForAccount`/`movementsForCategory`, since both queries select the identical column list), then both call a non-generic private `buildSplitDraft(lines: List<SplitLineRow>)` for the final construction. (An initial generic `buildSplitDraft<T>` with 5 accessor lambdas was flagged by `simplicity-guardian` as heavier than the duplication it removed and replaced with this simpler mapper-reference form.) | Low |
| **M14** | `movementsForCategory` duplicates `v_account_flow` sign logic ad hoc | Maintainability | **RESOLVED** P5R-9 | `movementsForCategory` (Android-local, `android/app/src/main/sqldelight/.../Movements.sq` — not in `shared/`) now joins `v_account_flow` by `movement_id` instead of hand-rolling the sign `CASE`, mirroring `accountFlowForAccount`; the now-redundant explicit `archived_at IS NULL` filter was dropped since `v_account_flow` already excludes archived movements in both its branches. Regression test added in `CoreLedgerRepositoryTest.kt` asserting the existing mapper (`::mapAccountFlowEntry`, already shared with `accountFlowForAccount`) still gets a correctly-signed `-2_500L` for the seeded expense | Low |
| **M15** | `docs/04-data-model.md` missing a `v_movement_summary` section | Maintainability | **RESOLVED** P5R-9 | Added `### 7.7 Movement summary (list/detail display rollup)` with a condensed representative `CREATE VIEW`, cross-referencing `shared/queries/v_movement_summary.sql` for the full definition and flagging that (unlike the other six views, each touched once at `P0A-3`) this view has been edited multiple times and must keep its embedded copy in `shared/migrations/002_add_splits_tag_id.sql` in sync on any future edit | Low |
| **C9** | Movement form type/payer switches silently destroy entered data | Critical | **RESOLVED (implementation)** P5R-17 WP1 | Segment re-tap guard, non-destructive normalization, and pre-save data-loss warning shipped in `2cf829e`; focused tests and full unit/build gate green; manual closure tracked in `docs/17` §5 | High |
| **F12** | Movement form misrepresents recurrence on edit | Functional | **RESOLVED (implementation)** P5R-17 WP3 | Real template cadence/status loads read-only; toggle-off offers end/unlink; quick-template carries notes; manual closure tracked in `docs/17` §5 | High |
| **F13** | Budgets unreachable through normal navigation | Functional | **RESOLVED (implementation)** P5R-17 WP4 | Gestió tile, category-flow CTA, notification retarget, and confirmed archive shipped; manual closure tracked in `docs/17` §5 | High |
| **F14** | Shared template's split invisible/uneditable in template form | Functional | P5R-17 | `docs/17` WP8 — read-only split summary + remove action (visibility half of the P4-2 deferred item; full editing stays deferred) | High |
| **U8** | Form validation errors render off-screen, no field identity | Usability | **RESOLVED (implementation)** P5R-17 WP2 | Field-level identity, error styling, and scroll-to-error shipped across forms; manual closure tracked in `docs/17` §5 | High |
| **U9** | Four forms still use free-text `YYYY-MM-DD` date entry | Usability | **RESOLVED (implementation)** P5R-17 WP5 | Refund, settlement, confirm-due, and budget forms use `FormDatePicker`; manual closure tracked in `docs/17` §5 | High |
| **U10** | Budget archive fires with no confirmation | Usability | **RESOLVED (implementation)** P5R-17 WP4 | Standard `archiveCandidate` confirmation shipped; manual closure tracked in `docs/17` §5 | High |
| **U11** | Settlements/refunds uneditable, with no explanation shown | Usability | **RESOLVED (explanation)** P5R-17 WP6a | Movement Detail now explains archive-and-recreate; WP6b direct editing remains optional and does not block closure | Medium |
| **U12** | Inconsistent interaction grammar across entity lists/forms | Usability | P5R-17 | `docs/17` WP7 — card-tap rule, add-button placement, chips→selects, person form as page, template transfer filter | High |
| **U13** | Template form dumps advanced knobs flat + status field in create | Usability | P5R-17 | `docs/17` WP8 — advanced fold; status removed from the form | High |
| **U14** | Split editor lacks assign-remainder helper; payer resets on edit | Usability | P5R-17 | `docs/17` WP9 — remainder action (UI-state only, `SplitCalculator` untouched); payer reset documented if schema carries no payer | Medium |
| **U15** | No inline category creation from the movement form | Usability | P5R-17 | `docs/17` WP10 — create-sentinel option mirroring the split editor's create-person | Medium |
| **U16** | Account reordering is menu-only (3 taps per position) | Usability | P5R-17 | `docs/17` WP11 — reorder mode with inline arrows; menu kept as accessibility path | Low |
| **M16** | Hardcoded Catalan UI strings (movement-form titles, account count) | Maintainability | P5R-17 | `docs/17` WP7 — externalize to `strings.xml` (+ plural) | High |
...
#### C9, F12–F14, U8–U16, M16 — P5R-17 data-entry UX audit (2026-07-10)
Findings from a dedicated UX audit of every add/edit flow (movement form + detail, accounts, categories, recurring, trips, tags, people/settlements, refunds, budgets, navigation shell), performed 2026-07-10 after `P5R-16`. All are owned by **`P5R-17`**; full evidence, impact, status, and step-level remediation live in `docs/17-data-entry-ux-remediation.md`. **Status audit 2026-07-16:** WP1 and WP3–WP6a are implemented in `2cf829e` and manually closed on isolated controlled data; WP2 C17-03 remains open because the exact invalid-state setup is unreachable from the current UI, while its automated validation paths are covered. WP7–WP11 remain open. Optional WP6b is deferred and is not a closure requirement. Only the critical finding's original detail is retained below:

- **C9** — `MovementsViewModel.normalizeForm` hard-clears state on every type change (switching to TRANSFER nulls category/split editor and latches `removeExistingSplit = true`; switching back restores nothing), and both segmented controls fire `onSelect` even for the already-selected segment, so idly re-tapping "Jo" while editing a *shared* expense maps to `PERSONAL` and wipes the split configuration — if saved, the stored split is removed with no warning. Switching an existing expense's payer to "Un altre" additionally drops payee/notes/one-time/recurrence silently (the external-split draft has no such fields). Same bug family as C8 (which fixed the *routing*; C9 is the *data-preservation* half). Reachable through the ordinary edit flow; regression tests must be verified failing against pre-fix code.

F12, F13, U8–U11 are now implemented as described in the master table. F14, U12–U16, and M16 remain functional gaps, consistency problems, and friction; none are data-corrupting. They resume after P5R-19 because the remaining work overlaps its movement-form, route-chrome, naming, and interaction-sweep surfaces.
- **Confidence:** High (every finding verified against the working tree on the audit date; file/line evidence recorded in the audit conversation and reproduced as needed in `docs/17`).
...
- **Tier 3 mapping:**
  ...
  - **P5R-9** (component consolidation): M8–M15 resolved; M7 partially resolved (see its row above — the two backing-shared-SQL methods were kept, not deleted, since they're still spec'd features awaiting UI wire-up, not dead code). F11 remains open, tracked separately.
  - **P5R-8** (settings/sync): DeviceAccessState placeholder (pre-existing).
  - **P5R-17** (data-entry UX remediation, `docs/17-data-entry-ux-remediation.md`): implementation shipped for C9, F12, F13, U8, U9, U10, and U11; controlled-data manual closure is complete for WP1 and WP3–WP6a, while WP2/C17-03 remains open because the exact invalid-state setup is unreachable from the current UI. F14, U12–U16, and M16 remain open. WP-level ownership stays WP1=C9, WP2=U8, WP3=F12, WP4=F13+U10, WP5=U9, WP6=U11, WP7=U12+M16, WP8=U13+F14, WP9=U14, WP10=U15, WP11=U16.
