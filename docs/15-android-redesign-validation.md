...
Analysis is 5 tabs — `Resum` · `Categories` · `Comparativa` (Compara) · `Històric` · `Fix/Var` — sharing one header with labeled `Abast`/`Valors` selectors, period controls, and filters. The tab row scrolls horizontally on narrow phones so labels are never clipped. Every top-level aggregate, chart, and row across all 5 tabs is display-only (§6 Aggregate Interaction Contract, `docs/11`): tapping never opens a movement list or trip detail, only filters/controls respond.
